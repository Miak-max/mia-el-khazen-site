...
- The hero is a **dense, overlapping collage of all fifteen projects** matching `Reference.png` (user's moodboard, 10 Sep 2026; git- and deploy-ignored): no titles or captions, pieces of very different sizes layered over each other and bleeding slightly past the edges, every piece at least ~62% visible. Each is `<a class="ph" href="#<slug>">` showing `images/<slug>/cover.jpg` then `01.jpg`. `tools/layout.py` generates separate desktop and phone arrangements (`hero.seed`, `hero.seed_m` in `tools/layouts.json`); `python3 tools/layout.py --reseed` searches for the most even arrangement.
